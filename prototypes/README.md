...
| P14 | `the-drift.html` | Can you *steer* an algorithm with your thumb? The field becomes a bounded territory: push into an edge with intent and the world flows under the thumb; dwell tilts what unseen ground materializes ahead; the walked world never rewrites; the journey is an object. Rev. 3: the ring of dots nearest the selection blooms into readable labels while the thumb is slow, and each kind has its own detent. Rev. 4 (private until filing): the map means something — right is WILD, left CALM, up SERIOUS, down GOOFY in every world; named regions sit at fixed coordinates as coloured nebulas; a four-word compass on the ring; a chip strip switches worlds and keeps your place; hue = region, lightness = read state, the intensity ramp on the nebula only | `docs/dot-field-drift.md` (the four-lens referee spec it implements; §8 rev. 3, §9 rev. 4) · `docs/dot-field-map.md` (the map ruling: axes, edition-0 tables, colour grammar, must-prove) | A fast skim through the edge band never moves the world; release stops it dead in a beat; back-scrubbing meets the exact same dots; the label names the place you are in; you read three neighbours while feeling one, and a skim never shows a label; a stranger says what "push right" does without being told |
