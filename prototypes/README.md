...
| P14 | `the-drift.html` | Can you *steer* an algorithm with your thumb? The field becomes a bounded territory: push into an edge with intent and the world flows under the thumb; dwell tilts what unseen ground materializes ahead; the walked world never rewrites; the journey is an object. Rev. 3: the ring of dots nearest the selection blooms into readable labels while the thumb is slow, and each kind has its own detent. Rev. 5 (private until filing): the map is a wheel — the hub is the mainstream, the rim is the deep end, each bearing is a kind; eight sectors carry the kinds and their hues; HOME and DEEPER on the ring turn with you; push and the depth word lights with the kind ahead ("DEEPER · HORROR"); a chip strip switches worlds and keeps your bearing and depth; five worlds including a staged movie catalogue on a genre wheel. Real worlds: an edition JSON from the shorts pipeline loads at boot and joins the strip — cells placed, not sampled; YouTube's own player on open; source, date and layout hash in the footer | `docs/dot-field-drift.md` (§8 rev. 3, §9 rev. 4, §10 rev. 5) · `docs/dot-field-map.md` (the ruling, the reversal, edition-0 tables, colour grammar, must-prove) · `docs/dot-field-shorts.md` (real content: the pipeline, placement, terms) | A fast skim through the edge band never moves the world; release stops it dead in a beat; back-scrubbing meets the exact same dots; the label names the place you are in; you read three neighbours while feeling one, and a skim never shows a label; a stranger says what "push away from the middle" does without being told |
