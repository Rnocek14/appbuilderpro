...
```mermaid
flowchart TB
    subgraph DEF["ONE CAPABILITY — defined once, versioned in the catalog"]
        CONTRACT["contract<br/>inputs · outputs · required connections (granted, scoped)"]
        GATESD["gates<br/>suppression · consent per send class · honesty holes · caps · approval classes"]
        CRIT["criteria pack + measurement contract"]
        WSDEF["its workshop<br/>bench archetype · Moves · palette wiring"]
    end

    subgraph SLIDER["THE DRIVE SLIDER — one environment, four pacings"]
        HAND["HAND<br/>a Studio session:<br/>bench · palette · counsel · moves"]
        ASK["ASK<br/>the Counsel performs one move;<br/>result lands on the bench as a variant"]
        HOFF["HAND OFF<br/>a worker continues the same session,<br/>guided by the Ledger; reports back;<br/>every step in Activity"]
        AUTO["AUTOMATE<br/>an Automation on a trigger:<br/>heartbeat trace · one-gesture pause ·<br/>never invisible"]
    end

    BIRTH["born: built-in · configured ·<br/>LEARNED (distilled sessions, gated) ·<br/>GENERATED (built app mounted back)"] --> DEF
    DEF --> HAND
    DEF --> ASK
    DEF --> HOFF
    DEF --> AUTO
    HAND ---|"slider, not a room change"| ASK
    ASK --- HOFF
    HOFF --- AUTO
    HOFF -->|"repeated hand-off distills:<br/>promotion proposal"| AUTO

    subgraph RECORD["ONE RECORD"]
        LEDGER["session Ledger —<br/>who did what, stamped:<br/>you · worker · routine"]
    end
    HAND --> LEDGER
    ASK --> LEDGER
    HOFF --> LEDGER
    ...
    subgraph EXIT["ONE GATE — every exit, every drive"]
        QUEUE["the Queue<br/>full decision context inline"]
        DIAL["earned-autonomy dial<br/>per capability × world × class<br/>auto-approve = standing grant,<br/>instantly revocable, rows stay visible"]
        QUEUE --- DIAL
    end
    LEDGER --> QUEUE
    QUEUE --> WORLD["the world: sends · publishes ·<br/>money · records"]
    WORLD --> OUTCOME["outcomes annotate artifacts →<br/>Playbook → sharper criteria"]
    OUTCOME -.->|"feeds"| CRIT
```
