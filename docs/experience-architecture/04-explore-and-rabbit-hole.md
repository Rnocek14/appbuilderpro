...
```mermaid
flowchart TB
    subgraph ENTRY["Entry — ceremony-free"]
        BAR["Open question at the Bar<br/>interpretation chip shows scope"]
        OBJ["'Explore this' on any object<br/>seeded with the object's rows"]
        INW["Curiosity inside a world,<br/>a mission, or the builder"]
    end

    BAR -->|"no home: silent birth"| RH["Curiosity world materializes<br/>(a rabbit hole — no form, no name asked)"]
    BAR -->|"resolves by meaning"| SESS
    OBJ --> SESS
    INW --> SESS
    RH --> SESS

    subgraph LIVE["The session — conversation + live map"]
        SESS["Dialogue grows the map:<br/>concepts · sources · questions ·<br/>theories · discoveries · media"]
        FORK["Branches fork<br/>(deliberate or detected, undoable)"]
        BEAC["Beacons parked:<br/>name the gap, hold the guess"]
        COMB["Branches combine:<br/>synthesis, contradictions surfaced"]
        EXP["Experiments: inline sims<br/>and Lab sessions"]
        MAKE["Make verbs: videos, visuals,<br/>docs, datasets, apps"]
        WS["Deepen: Theory Workshop / Lab /<br/>any craft — map rides in the Palette"]
        SESS --> FORK --> BEAC
        FORK --> COMB
        SESS --> EXP
        SESS --> MAKE
        SESS --> WS
        WS -->|"commits return as map nodes"| SESS
        EXP -->|"evidence edges, predictions closed"| SESS
    end

    SESS -->|"untouched 2 weeks"| COOL["Cooling — leaves Continue,<br/>joins the quiet band"]
    COOL -->|"untouched 6 weeks"| DORM["Dormant — absent from the Field,<br/>findable by meaning forever"]
...
```mermaid
flowchart LR
    subgraph ONE["ONE WORLD, ONE IDENTITY — nothing ever moves, nothing is ever copied"]
        direction LR
        SEED["SEED<br/>curiosity dressing<br/>map territory grows ·<br/>beacons · theories ·<br/>no clock, no counterparty"]
        DEEP["DEEPENED<br/>workshop sessions ·<br/>Lab experiments ·<br/>artifacts + prototype ·<br/>optional inbound watch"]
        PROM["PROMOTED (venture)<br/>areas mount around the map ·<br/>first mission staged ·<br/>beacons become asks ·<br/>predictions become tracked calls"]
        OPER["OPERATING<br/>money + counterparty layers ·<br/>grants + isolation review ·<br/>automations on the clock ·<br/>map remains the knowledge core"]
        SEED -->|"'take this into a workshop'"| DEEP
        DEEP -->|"Proposal → Charter<br/>(one confirm)"| PROM
        PROM -->|"heavier rungs ask more:<br/>grants, isolation"| OPER
        OPER -.->|"demotion: shed layers,<br/>lose nothing"| SEED
    end

    MAP["THE MAP<br/>persists through every stage:<br/>the record, then the floor plan,<br/>then the knowledge core"]
    MAP --- SEED
    MAP --- DEEP
    MAP --- PROM
    MAP --- OPER
