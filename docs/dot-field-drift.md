...
## 10. Revision 5 — the wheel (2026-09-02, the same day)

The founder read rev. 4 and clarified: *"deeper or shallower drift determines intensity and then
direction determines category … stay in that genre and play around with similar content or more
intense, less intense, similar sport."* That is a polar map, and it removed the panel's reason for the
plane (it had assumed *tone* on the angle). Rev. 5 makes the map a wheel: eight sectors 45° apart
carry a world's kinds and their hues (one hue per sector, exact — the collision rule is gone); regions
sit at (sector, radius); the hub is the mainstream and the rim is the deep end in the world's own
words (casual → extreme, mainstream → deep cut …); register (serious / goofy) is a tag on the card,
not geometry. The ring shows HOME at the bearing to the hub and DEEPER opposite; while you push, the
depth word you head toward lights with the kind ahead at the screen edge ("DEEPER · HORROR"); the
first ring touch flies the eight kinds out to their bearings once. The camera is clamped to the wheel
— the rim is the wall ("as extreme as this world goes"), beyond it dark land. Steering is decomposed
into radial and tangential displacement, one explicit event per 4 cells, with the heading recorded so
"deeper" from the hub favours the kind under the thumb. A boundary tick fires only into a *decisive*
place (argmax share ≥ 0.34), so the blended hub never chatters. A fifth world, **movies**, staged on a
genre wheel from a hand-written list of real titles, is where the remote is most literally a remote.
The reversal, its reasons and its costs are in **`docs/dot-field-map.md` §0b**; the edition-0 tables
there are generated by `scripts/drift-edition-tables.ts`. Still private until the provisional is filed.
