...
- **Calendar.** `social_posts` rows already carry `scheduleAt`, status, platforms, world_id;
  ...
  (`capabilitySeed.ts:25`). §7 specs the surface.
...
- **Direct-manipulation surface**: the existing nine-bench grammar carries the making — social
  ...
  CONTENT-CALENDAR surface (§6) for time.
...
- **Verdict**: **REUSABLE-FRAMEWORK.** The social studio flavor, board bench, producers, and
  ...
  is missing is one deep surface (the calendar, §6), two rails (onboarding plumbing, engagement
...
| 8 | Domain Workshop | Social/Content Workshop (REUSABLE-FRAMEWORK, §5) + Content-Calendar surface (DEEP-ENVIRONMENT, §6 — the thin one) |
