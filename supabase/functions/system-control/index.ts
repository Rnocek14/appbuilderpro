...
const SECRETS: { name: string; pillar: string; unlocks: string }[] = [
  ...
  { name: 'ANTHROPIC_API_KEY', pillar: 'intelligence', unlocks: 'Server-side model calls (agent-turn relay, workers, follow-up + reply drafters)' },
  { name: 'OPENAI_API_KEY', pillar: 'intelligence', unlocks: 'Optional: concept imagery in demo sites (gpt-image-1) + default voiceover TTS — pitches still send without it' },
