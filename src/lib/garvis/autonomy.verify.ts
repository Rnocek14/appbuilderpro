...
import { classifyApproval, computeStreak, eligibleForAuto, MIN_CLEAN_STREAK, COLD_PITCH_CLEAN_STREAK, minStreakFor, defaultDailyCap, AUTONOMY_CLASSES, GRANTABLE_CLASSES } from './autonomy';
...
check('a COLD pitch (kind marker) is its OWN class — never a followup', classifyApproval('send_email', { campaign_id: 'c', message_id: 'm', kind: 'cold_site_pitch' }) === 'cold_pitch');
...
check('five classes, cold pitch last and explicit', AUTONOMY_CLASSES.length === 5 && AUTONOMY_CLASSES[4].id === 'cold_pitch');
check('the cold-pitch bar is five times the ordinary bar', COLD_PITCH_CLEAN_STREAK === 5 * MIN_CLEAN_STREAK && minStreakFor('cold_pitch') === COLD_PITCH_CLEAN_STREAK && minStreakFor('followup') === MIN_CLEAN_STREAK);
check('cold-pitch eligibility needs the higher bar', !eligibleForAuto(MIN_CLEAN_STREAK, 'cold_pitch') && !eligibleForAuto(COLD_PITCH_CLEAN_STREAK - 1, 'cold_pitch') && eligibleForAuto(COLD_PITCH_CLEAN_STREAK, 'cold_pitch'));
check('ordinary classes keep the ordinary bar', eligibleForAuto(MIN_CLEAN_STREAK, 'invoice_chase'));
check('default caps are small everywhere', AUTONOMY_CLASSES.every((c) => defaultDailyCap(c.id) <= 5));
check('the cold pitch is classified but NOT grantable — no server path self-approves a first email', !GRANTABLE_CLASSES.includes('cold_pitch') && GRANTABLE_CLASSES.length === 4);
