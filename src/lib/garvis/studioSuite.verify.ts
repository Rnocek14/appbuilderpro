...
const SPECS: StudioSpec[] = [EMAIL_SPEC, ADS_SPEC, COPY_SPEC];
...
// --- honesty across the board: unknowns are EDIT holes, never invented ----------------------
{
  ...
  for (const spec of SPECS) {
    ...
    check(`[${spec.kind}] bare context → visible [EDIT] holes, no fabricated business name`, /\[EDIT:/.test(blob) && /\[EDIT: your business\]/.test(blob));
  }
}
