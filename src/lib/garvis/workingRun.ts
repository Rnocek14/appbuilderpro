...
// discovery invisible entirely). These loaders are all fail-soft: a missing table (migration not
// applied on this install) reads as an honest 'not set up', never a crash, and NOTHING here
// mutates — actions stay on their owning pages.

import { supabase } from '../supabase';

const MISSING_RE = /does not exist|relation|schema cache/i;

/** null = table missing on this install (migration not applied) — distinct from an empty list. */
...
export interface BuildJobLite {
  id: string; name: string | null; status: string; phase: string | null; pause_reason: string | null; created_at: string;
}
export function loadBuildJobs(): Promise<Loaded<BuildJobLite>> {
  return failSoft(async () => {
    const { data, error } = await supabase.from('jobs')
      .select('id, name, status, phase, pause_reason, created_at')
...
export async function loadDiscoveredCounts(): Promise<DiscoveredCounts | null> {
  try {
...
export async function loadAutomationSummary(): Promise<AutomationSummary | null> {
  try {
    const count = async (status: string) => {
...
export async function loadReelCounts(): Promise<ReelCounts | null> {
  try {
