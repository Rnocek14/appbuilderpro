...
export const ACTION_SPECS: ActionSpec[] = [
  {
    ...
    category: 'company',
    risk: 'spend',
    description: 'Run business genesis on a one-line venture intent: DNA synthesis → a designed company (chartered areas, seeded playbooks, opening play) as a REVIEWABLE DRAFT. Use once per distinct company in the intent — for the OPERATOR\'S OWN ventures only. Doing work FOR someone else\'s business is onboard_client, never this.',
  ...
  },
  {
    ...
    category: 'company',
    risk: 'spend',
    description: 'Open a CLIENT ENGAGEMENT: a business the operator runs work FOR (their marketing, paperwork, site — the scope). Creates the engagement with a deterministic intake checklist derived from the scope (what to collect from the client) and drafts the CLIENT\'S world through the normal approval ceremony. Use whenever the intent is doing work for someone else\'s business — never found_company for that.',
    params: [
      { name: 'client_name', required: true, hint: 'who the client is ("Jane Smith")' },
      { name: 'business', required: true, hint: 'their business, in one line ("residential realty in Madison")' },
      { name: 'scope', required: true, hint: 'what the operator does for them ("marketing + listing paperwork")' },
      { name: 'email', required: false, hint: 'the client\'s email' },
    ],
    produces: 'an engagement in the Client book (intake checklist ready) + the client\'s world draft awaiting review on Businesses',
  },
  {
    id: 'research_market',
