...
export async function queuePitch(input: QueuePitchInput): Promise<{ approvalId: string; messageId: string }> {
  ...
  // SELECT-FIRST, never a blind upsert: an existing contact keeps its email_status untouched.
  // A blind upsert here would reset 'unsubscribed'/'bounced'/'complained' back to 'unknown' —
  // suppression is sacred and is never reset (the same guard inboxRun/workwebRun/site-events use).
  ...
  if (existing) {
    ...
    // Race-safe: uq_contacts_owner_email (app_0025) makes a concurrent insert conflict; on
    // conflict we re-select rather than overwrite anything.
    ...
    if (c) contactId = (c as { id: string }).id;
      ...
      const { data: again } = await supabase.from('contacts')
        .select('id').eq('owner_id', uid).eq('email', to).maybeSingle();
      if (!again) throw new Error(cErr?.message ?? 'Could not save the contact.');
