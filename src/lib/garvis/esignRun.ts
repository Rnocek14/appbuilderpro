...
export interface PaperworkTemplate {
  ...
  fields: { token: string; label: string; hint: string }[];
}

/** Scope a query to one business without hiding legacy unscoped rows: world-stamped rows show
 *  only in their world; rows that predate world stamping (world_id null) stay visible anywhere.
 *  This is the isolation contract for every paperwork read — client A's world-scoped documents
 *  can never surface in client B's studio. */
function worldScope(worldId?: string | null): string | null {
  return worldId ? `world_id.eq.${worldId},world_id.is.null` : null;
}
...
export async function listTemplates(worldId?: string | null): Promise<PaperworkTemplate[]> {
  let q = supabase.from('paperwork_templates')
    .select('id, name, doc_kind, body, fields, updated_at').order('updated_at', { ascending: false }).limit(50);
  const scope = worldScope(worldId);
  if (scope) q = q.or(scope);
  const { data, error } = await q;
  if (error) throw new Error(error.message);
  return ((data ?? []) as PaperworkTemplate[]).map((t) => ({ ...t, fields: Array.isArray(t.fields) ? t.fields : [] }));
}

export async function saveTemplate(input: {
  id?: string; name: string; docKind?: string; body: string; worldId?: string | null;
  fields?: { token: string; label: string; hint: string }[];
}): Promise<PaperworkTemplate> {
  ...
  const row = {
    ...
    doc_kind: input.docKind ?? 'agreement', body: input.body, fields: input.fields ?? [],
    updated_at: new Date().toISOString(),
  };
  const q = input.id
    ? supabase.from('paperwork_templates').update(row).eq('id', input.id).select('id, name, doc_kind, body, fields, updated_at').single()
    : supabase.from('paperwork_templates').insert(row).select('id, name, doc_kind, body, fields, updated_at').single();
  const { data, error } = await q;
  if (error || !data) throw new Error(`Could not save the template: ${error?.message ?? 'unknown'}`);
  const saved = data as PaperworkTemplate;
  return { ...saved, fields: Array.isArray(saved.fields) ? saved.fields : [] };
}
...
export async function searchContacts(q: string, worldId?: string | null): Promise<ContactHit[]> {
  const needle = q.trim();
...
export async function listEnvelopes(limit = 12, worldId?: string | null): Promise<EnvelopeRow[]> {
...
export async function extractPaperworkTemplate(sampleText: string, hint?: string): Promise<ExtractedTemplate> {
  ...
  const { data, error } = await supabase.functions.invoke('cluster-chat', {
    body: {
      ...
      // format:'raw' — the extractor defines its own JSON contract; the studio decision suffix
      // was contradiction noise here too.
      format: 'raw',
    },
  });
  if (error) throw new Error(error.message);
