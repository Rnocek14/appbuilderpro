      ...
      <svg className="mc-links" viewBox="0 0 100 100" preserveAspectRatio="none" aria-hidden="true">
        <defs>
          <linearGradient id="mc-lg" x1="0" y1="0" x2="1" y2="1">
            <stop offset="0" stopColor="#F5813E" /><stop offset="1" stopColor="#C77BE0" />
          </linearGradient>
        </defs>
        {pos.map((p, i) => (
...
.mc-root{ position:relative; width:100%; height:min(72vh,640px); border-radius:22px; overflow:hidden;
  background:
    radial-gradient(1000px 640px at 50% 44%, rgba(245,129,62,.12), transparent 60%),
    radial-gradient(820px 520px at 80% 82%, rgba(199,123,224,.10), transparent 55%),
...
.mc-node{ position:absolute; transform:translate(-50%,-50%); cursor:pointer; background:none; border:none; padding:0;
  display:flex; flex-direction:column; align-items:center; gap:7px; width:128px; z-index:2; color:#F1E9DD; font:inherit;
  transition:transform .25s cubic-bezier(.2,.7,.2,1); }
.mc-node:hover{ transform:translate(-50%,-50%) scale(1.06); }
.mc-node:focus-visible{ outline:none; }
.mc-node:focus-visible .mc-orb{ box-shadow:0 0 0 2px #F5813E, 0 0 34px -4px rgba(245,129,62,.6); }
.mc-node.dim{ opacity:.5; }
.mc-orb{ width:84px; height:84px; border-radius:22px; background:#221B2B; border:1px solid #3A2E47;
  display:grid; place-items:center; position:relative; box-shadow:0 10px 30px -12px rgba(0,0,0,.6); transition:.25s ease; }
.mc-node:hover .mc-orb{ border-color:#F5813E; box-shadow:0 0 0 1px #F5813E, 0 0 34px -4px rgba(245,129,62,.55); }
.mc-orb.violet{ border-color:#4a3a52; }
.mc-node:hover .mc-orb.violet{ border-color:#C77BE0; box-shadow:0 0 0 1px #C77BE0, 0 0 34px -4px rgba(199,123,224,.5); }
.mc-em{ font-size:29px; line-height:1; }
.mc-lab{ font-size:13px; font-weight:600; }
.mc-sub{ font-size:11px; color:#A99BB0; margin-top:-3px; }
.mc-count{ position:absolute; top:-6px; right:-6px; min-width:20px; height:20px; padding:0 5px; border-radius:999px;
  background:linear-gradient(150deg,#F5813E,#F2A968); color:#241a12; font-size:11px; font-weight:700; display:grid; place-items:center; box-shadow:0 2px 8px rgba(0,0,0,.4); }

.mc-center{ z-index:3; width:196px; }
.mc-center .mc-orb{ width:158px; height:158px; border-radius:36px;
  background:radial-gradient(120px 120px at 40% 30%, rgba(245,129,62,.28), transparent 70%), #221B2B;
  border-color:#4a3a2c; box-shadow:0 0 0 1px rgba(245,129,62,.35), 0 0 60px -6px rgba(245,129,62,.55); }
.mc-center:hover .mc-orb{ box-shadow:0 0 0 1px rgba(245,129,62,.6), 0 0 70px -6px rgba(245,129,62,.7); }
.mc-cin{ text-align:center; padding:0 14px; display:flex; flex-direction:column; gap:4px; }
.mc-ck{ font-size:10px; letter-spacing:.18em; text-transform:uppercase; color:#F5813E; font-weight:700; }
...
.mc-cs{ font-size:11px; color:#A99BB0; font-variant-numeric:tabular-nums; }
...
.mc-sat{ position:absolute; width:13px; height:13px; border-radius:50%; transform:translate(-50%,-50%); z-index:1;
  background:radial-gradient(circle at 40% 35%, #F2A968, #F5813E); box-shadow:0 0 12px -1px rgba(245,129,62,.6); animation:mc-pop .4s ease; }
