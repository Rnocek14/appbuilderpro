...
export function StandingOrdersPanel({ worldId, onToast }: {
  ...
  const add = async () => {
    ...
    setBusy('create');
    try {
      await createOrder({ worldId, kind, label, cadence, url: kind === 'watch_url' ? url : undefined });
  ...
  return (
    <div className="mt-4 rounded-2xl border border-forge-border bg-forge-panel/40 p-4">
      ...
      <p className="text-xs text-forge-dim">
        Recurring checks that run on their own — watch a page for changes, or digest this world on a
        cadence. <span className="text-forge-ink/80">They only read and record; nothing is ever sent for you.</span>
      </p>
      ...
      {showForm && (
        <div className="mt-3 rounded-lg border border-forge-border bg-forge-raised/20 p-3">
          <div className="flex flex-wrap gap-1.5">
            ...
            <button onClick={() => setKind('cadence_digest')} className={`flex items-center gap-1 rounded-full border px-2.5 py-1 text-[11px] ${kind === 'cadence_digest' ? 'border-forge-ember/60 bg-forge-ember/10 text-forge-ember' : 'border-forge-border text-forge-dim'}`}><CalendarClock size={11} /> Digest this world</button>
      ...
      {loading ? (
        ...
        !showForm && <p className="mt-3 text-xs text-forge-dim/60">No standing orders yet — set one and this world starts checking things on its own clock.</p>
      ) : (
            ...
            <li key={o.id} className="rounded-lg border border-forge-border bg-forge-raised/20 px-3 py-2">
              <div className="flex items-center gap-2">
                ...
                <div className="flex shrink-0 items-center gap-1">
                  <button title="Run now" onClick={() => void act(o.id, async () => { const r = await runOrderNow(o.id); onToast('success', r.changed > 0 ? 'Ran — something changed; check your waking moment.' : r.failed > 0 ? 'Ran — the check failed; see its status line.' : 'Ran — no change.'); })}
