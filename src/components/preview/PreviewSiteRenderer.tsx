  ...
  return (
    <div className={`pv-site min-h-screen bg-[hsl(var(--bg))] antialiased ${shot ? 'pv-shot' : ''} ${flair.map((f) => `pv-f-${f}`).join(' ')}`} style={vars}>
      ...
      {/* MOBILE CALL BAR — most owners open the pitch on a phone; the strongest conversion lever
          ...
          Sits ABOVE the collapsed ClaimBar (which pins to bottom-0 at z-50) so the two don't stack
          on top of each other and hide the call button. */}
      {!shot && phone && (
        <div className="fixed inset-x-0 bottom-[4.75rem] z-40 border-y border-[hsl(var(--bor))] bg-[hsl(var(--bg)/0.94)] px-4 pb-2.5 pt-2.5 backdrop-blur md:hidden">
